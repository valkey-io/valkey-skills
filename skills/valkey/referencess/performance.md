...
|----------------|------|-----------------|
| Memory encoding and key sizing | `performance-memory-encoding.md` | listpack, hashtable, skiplist, intset, quicklist, embstr, top-level key overhead, hash-bucketing, size limits, OBJECT ENCODING, bitmaps, large values |
...
| Throughput, pipelining, and hot keys | `performance-throughput.md` | lazyfree defaults, SCAN semantics, pipelining, connection pooling, io-threads, events-per-io-thread, hot key, CLIENT TRACKING |
| Benchmarking | `performance-benchmarking.md` | valkey-benchmark, memtier_benchmark, pipeline depth, client threads, warm-up, R/W mix |
