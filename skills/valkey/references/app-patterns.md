...
|-----------|------|-----------------|
| Cache-aside, stampede prevention, refresh locks, TTL jitter, shared-TTL bulk set, explicit invalidation, cache eviction policy | `app-caching.md` | cache-aside, lazy loading, stampede, early refresh, TTL jitter, MSETEX, UNLINK, allkeys-lru, volatile-lru, write-through, write-behind |
...
| Login/session storage | `app-sessions.md` | session hash, sliding TTL, session rotation, HGETEX, HGETDEL, one-time fields, per-field TTL, concurrent sessions, session ID entropy |
