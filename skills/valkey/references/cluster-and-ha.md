...
|-----------|------|-----------------|
| Cluster key design and client behavior | `cluster-key-client-behavior.md` | hash slot, CRC16, hash tag, CROSSSLOT, MOVED, ASK, READONLY, read-from-replica, cluster pipeline, CLUSTERSCAN, SCAN every primary, sharded pub/sub, numbered DBs |
| Slot migration and resharding | `cluster-slot-migration.md` | CLUSTER MIGRATESLOTS, GETSLOTMIGRATIONS, remaining_repl_size, CANCELSLOTMIGRATIONS, no ASK window, reshard, --cluster-use-atomic-slot-migration, slot migration state |
